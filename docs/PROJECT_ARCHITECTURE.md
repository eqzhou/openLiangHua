...
- [SYSTEM_ARCHITECTURE_BLUEPRINT_2026-04-04.md](D:/openlianghua/docs/SYSTEM_ARCHITECTURE_BLUEPRINT_2026-04-04.md)
- [MIGRATION_PLAN_2026-04-04.md](D:/openlianghua/docs/MIGRATION_PLAN_2026-04-04.md)

Use those two files as the current decision documents for the React-first migration. This file remains the durable inventory of modules, data flow, and runtime behavior.
...
- As of `2026-04-03`, the system is only partially database-backed:
  - authentication/session state now uses Postgres (`quant_db`, tables `app_users` and `app_sessions`)
  - research datasets and dashboard payloads still load from YAML / CSV / Parquet / Markdown through `config_repository.py` and `report_repository.py`
  - a future data-layer migration is still needed if the full research workspace should become database-driven
...
## 11. Database Layer Status (2026-04-03)

The system is now split into three storage tiers:

- Raw acquisition / generated files
  - parquet, csv, json, markdown files remain the production-of-record outputs generated by research scripts
- Postgres application/auth layer
  - `app_users`
  - `app_sessions`
- Postgres dashboard artifact layer
  - `dashboard_artifacts`
  - stores JSON payloads, text notes, and binary parquet blobs used by the app layer

Current repository behavior:

- `config_repository.py` prefers Postgres-backed config artifacts, then falls back to YAML files.
- `report_repository.py` prefers Postgres-backed report artifacts, then falls back to files.
- Core panels `daily_bar`, `feature_panel`, and `label_panel` can now load from Postgres parquet blobs.

Current write behavior:

- research/report generation still writes normal workspace files first
- the dashboard sync layer then mirrors supported artifacts into Postgres
- sync entrypoints:
  - `python -m src.db.dashboard_sync`
  - `powershell -ExecutionPolicy Bypass -File .\scripts\sync_dashboard_db.ps1`

Realtime quote storage behavior:

- realtime quote snapshots now have a dedicated Postgres path instead of only existing in request memory
- schema objects:
  - `realtime_quote_batches`
  - `realtime_quote_rows`
- schema script:
  - `db/sql/004_realtime_quote_snapshots.sql`
- runtime store:
  - `src/db/realtime_quote_store.py`
- orchestration:
  - `src/app/services/realtime_quote_service.py`
- request behavior:
  - during trading hours, requests still try live providers first and persist the latest successful snapshot
  - after `15:00` Asia/Shanghai, the system writes one same-day `post_close` snapshot and reuses that database snapshot for later same-day requests
  - if providers fail, the service can fall back to the latest stored same-day snapshot
- app-layer read behavior:
  - `dashboard_facade.get_service_payload()` now merges streamlit runtime status with a database-backed realtime snapshot summary
  - `get_shell_payload()` reuses that same service contract, so both the React workbench and service page can show identical `快照类型 / 抓取时间 / 覆盖率 / 失败股票` state
  - frontend presentation helpers for realtime state are centralized in `web/src/lib/realtime.ts`
