...
class AppLayerTests(unittest.TestCase):
    ...
    def test_watchlist_service_builds_holdings_with_discussion_and_inference_rank(self) -> None:
                ...
                "focus_pool": [
                    {
                        "ts_code": "000001.SZ",
                        "name": "Other",
                        "note": "Focus for breakout follow-up.",
                    }
                ],
            },
            daily_bar=daily_bar,
        ...
        self.assertEqual(len(watchlist_view), 2)
        holding_row = watchlist_view.loc[watchlist_view["ts_code"] == "000078.SZ"].iloc[0]
        ...
        self.assertEqual(int(holding_row["ensemble_rank"]), 1)
        self.assertEqual(int(holding_row["inference_ensemble_rank"]), 2)
        self.assertTrue(bool(holding_row["is_overlay_selected"]))
        self.assertTrue(bool(holding_row["is_inference_overlay_selected"]))
        self.assertEqual(int(holding_row["llm_round_count"]), 2)
        self.assertEqual(int(holding_row["llm_success_round_count"]), 1)
        discussion_snapshot = holding_row["llm_discussion_snapshot"]
        round_summaries = [
        ...
        self.assertIn("3.45", str(holding_row["premarket_plan"]))
