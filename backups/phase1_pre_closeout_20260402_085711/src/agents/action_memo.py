...
def _memo_scenarios(snapshot: dict[str, object]) -> list[str]:
    ...
    return [
        ...
        "条件：",
        "",
        f"- 冲高后反复开板/回落，但大部分时间还能守住 {defensive_price}",
        "- 午后仍有反抽，但没有形成持续加速",
        "",
        "执行理解：",
        "",
...
def _compose_action_memo(snapshot: dict[str, object]) -> str:
    ...
    lines = [
        ...
        f"- 参考浮亏：约 `{int(round(float(snapshot.get('unrealized_pnl') or 0.0))):,} 元`" if snapshot.get("unrealized_pnl") is not None else "- 参考浮亏：`-`",
        f"- 参考浮亏比例：约 `{fmt_pct(snapshot.get('unrealized_pnl_pct'))}`",
        "",
        "说明：",
        "",
...
def generate_action_memos(root: Path | None = None) -> list[Path]:
    ...
    for item in watchlist:
        ...
        output_path = reports_dir / f"{base_code}_action_memo_{plan_date}.md"
        save_text(_compose_action_memo(snapshot), output_path)
