...
def resolve_data_source(root: Path | None = None) -> str:
    ...
    universe = load_yaml(resolved_root / "config" / "universe.yaml")
    return normalize_data_source(universe.get("data_source", "akshare"))


def read_prediction_latest_snapshot(path: Path, usecols: list[str]) -> pd.DataFrame:
    if not path.exists():
