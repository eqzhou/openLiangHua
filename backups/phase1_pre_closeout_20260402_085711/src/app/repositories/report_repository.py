...
def load_dataset_summary(root: Path | None = None, *, data_source: str) -> dict[str, object]:
...
def load_feature_panel(root: Path | None = None, *, data_source: str) -> pd.DataFrame:
    resolved_root = root or project_root()
...
def load_label_panel(root: Path | None = None, *, data_source: str) -> pd.DataFrame:
    resolved_root = root or project_root()
    path = source_or_canonical_path(resolved_root / "data" / "labels", "label_panel.parquet", data_source)
    return _read_parquet_frame(path)


def load_daily_bar(root: Path | None = None, *, data_source: str) -> pd.DataFrame:
    resolved_root = root or project_root()
    path = source_or_canonical_path(resolved_root / "data" / "staging", "daily_bar.parquet", data_source)
    return _read_parquet_frame(path)


def load_metrics(root: Path | None = None, *, data_source: str, model_name: str, split_name: str) -> dict[str, Any]:
    resolved_root = root or project_root()
...
def load_stability(root: Path | None = None, *, data_source: str, model_name: str) -> dict[str, Any]:
    resolved_root = root or project_root()
    path = source_or_canonical_path(
        resolved_root / "reports" / "weekly",
...
def load_portfolio(root: Path | None = None, *, data_source: str, model_name: str, split_name: str) -> pd.DataFrame:
    resolved_root = root or project_root()
    path = source_or_canonical_path(
        resolved_root / "reports" / "weekly",
...
def load_predictions(root: Path | None = None, *, data_source: str, model_name: str, split_name: str) -> pd.DataFrame:
    resolved_root = root or project_root()
    path = source_or_canonical_path(
        resolved_root / "reports" / "weekly",
...
def load_feature_importance(root: Path | None = None, *, data_source: str, model_name: str) -> pd.DataFrame:
    resolved_root = root or project_root()
    path = source_or_canonical_path(
        resolved_root / "reports" / "weekly",
...
def load_overlay_candidates(root: Path | None = None, *, data_source: str) -> pd.DataFrame:
    resolved_root = root or project_root()
    path = source_or_canonical_path(resolved_root / "reports" / "weekly", "overlay_latest_candidates.csv", data_source)
    return _read_csv_frame(path)


def load_overlay_packet(root: Path | None = None, *, data_source: str) -> dict[str, Any]:
    resolved_root = root or project_root()
    path = source_or_canonical_path(resolved_root / "reports" / "weekly", "overlay_latest_packet.json", data_source)
    return read_json(path)


def load_overlay_brief(root: Path | None = None, *, data_source: str) -> str:
    resolved_root = root or project_root()
    path = source_or_canonical_path(resolved_root / "reports" / "weekly", "overlay_latest_brief.md", data_source)
    return read_text(path)


def load_overlay_inference_candidates(root: Path | None = None, *, data_source: str) -> pd.DataFrame:
    resolved_root = root or project_root()
    path = source_or_canonical_path(
        resolved_root / "reports" / "weekly",
...
def load_overlay_inference_packet(root: Path | None = None, *, data_source: str) -> dict[str, Any]:
    resolved_root = root or project_root()
    path = source_or_canonical_path(
        resolved_root / "reports" / "weekly",
...
def load_overlay_inference_brief(root: Path | None = None, *, data_source: str) -> str:
    resolved_root = root or project_root()
    path = source_or_canonical_path(
        resolved_root / "reports" / "weekly",
