...
def build_watchlist_base_frame() -> pd.DataFrame:
    ...
    frame = build_watchlist_view(
        ...
        daily_bar=repo_load_daily_bar(ROOT, data_source=ACTIVE_DATA_SOURCE),
        ridge_predictions=repo_load_predictions(ROOT, data_source=ACTIVE_DATA_SOURCE, model_name="ridge", split_name="test"),
        lgbm_predictions=repo_load_predictions(ROOT, data_source=ACTIVE_DATA_SOURCE, model_name="lgbm", split_name="test"),
        ensemble_predictions=repo_load_predictions(ROOT, data_source=ACTIVE_DATA_SOURCE, model_name="ensemble", split_name="test"),
        overlay_candidates=repo_load_overlay_candidates(ROOT, data_source=ACTIVE_DATA_SOURCE),
        ensemble_inference_predictions=repo_load_predictions(ROOT, data_source=ACTIVE_DATA_SOURCE, model_name="ensemble", split_name="inference"),
        overlay_inference_candidates=repo_load_overlay_inference_candidates(ROOT, data_source=ACTIVE_DATA_SOURCE),
    )
    sync_watchlist_snapshot_artifact(
        root=ROOT,
        data_source=ACTIVE_DATA_SOURCE,
...
def build_candidate_snapshot(model_name: str, split_name: str) -> pd.DataFrame:
    ...
    predictions = repo_load_predictions(ROOT, data_source=ACTIVE_DATA_SOURCE, model_name=model_name, split_name=split_name)
...
def build_factor_explorer_snapshot(field_explanations_items: tuple[tuple[str, str], ...]) -> dict[str, object]:
    ...
    feature_panel = repo_load_feature_panel(ROOT, data_source=ACTIVE_DATA_SOURCE)
