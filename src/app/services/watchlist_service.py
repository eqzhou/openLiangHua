...
def build_watchlist_view(
    *,
    ...
    if not holdings and not focus_pool:
        return pd.DataFrame()
    ...
    for entry_group, items in (("持仓", holdings), ("重点关注", focus_pool)):
        for item in items:
            ...
            if entry_group != "持仓":
                normalized_item.setdefault("cost", 0.0)
                normalized_item.setdefault("shares", 0)
...
def build_reduce_plan(row: pd.Series) -> pd.DataFrame:
    ...
    stages = [
        ...
        ("阶段二", float(row.get("breakeven_price") or 0.0), 0.40, "接近回本区后观察抛压"),
